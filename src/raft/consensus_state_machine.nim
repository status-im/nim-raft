...
func findFollowerProggressById(sm: RaftStateMachineRef, id: RaftNodeId): Option[RaftFollowerProgressTrackerRef] =
  return sm.leader.tracker.find(id)

func sendToImpl*(sm: RaftStateMachineRef, id: RaftNodeId, request: RaftRpcAppendRequest) =
...
func sendToImpl*(sm: RaftStateMachineRef, id: RaftNodeId, request: RaftInstallSnapshot) =
...
func addEntry(sm: RaftStateMachineRef, entry: LogEntry): LogEntry = {.cast(noSideEffect).}:
  var tmpEntry = entry
  if not sm.state.isLeader:
...
func addEntry*(sm: RaftStateMachineRef, config: RaftConfig): LogEntry = {.cast(noSideEffect).}:
...
func appendEntryReply*(sm: RaftStateMachineRef, fromId: RaftNodeId, reply: RaftRpcAppendReply) =
  if not sm.state.isLeader:
...
func appendEntry*(sm: RaftStateMachineRef, fromId: RaftNodeId, request: RaftRpcAppendRequest) =
...
func requestVote*(sm: RaftStateMachineRef, fromId: RaftNodeId, request: RaftRpcVoteRequest) =
  # D-Raft-Extended 
...
func requestVoteReply*(sm: RaftStateMachineRef, fromId: RaftNodeId, request: RaftRpcVoteReply) = 
  if not sm.state.isCandidate:
...
func installSnapshotReplay(sm: RaftStateMachineRef, fromId: RaftnodeId, replay: RaftSnapshotReply) =
